...
| `P2D_PORTS` | 否 | 候选池中随机 3–5 个 | 逗号分隔的固定 TCP 端口；留空启用自动选择 |
...
不设置或留空 `P2D_PORTS` 时，客户端会打乱
[`defaults.go`](internal/client/defaults.go) 中的候选池，每次启动随机确定 3–5 个监听
端口。若选中的端口已被其他进程占用，会继续从已打乱的剩余候选中补选，直到达到目标
数量；候选池不足或发生权限不足等其他绑定错误时，会关闭已打开的监听器并退出。

如需固定端口，可显式设置。例如同时监听 139、445、1080 和 1099：
...
显式指定端口时不进行替补；任意一个端口绑定失败都会关闭已经打开的监听器并整体退出，
避免静默进入部分监听状态。完整模板见
[config/client.env.example](config/client.env.example)。
