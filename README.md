...
- Go 客户端可一次监听多个 TCP 端口；未指定时从内置候选池随机选择 3–5 个；
- 自动选端口时若候选已被占用，会从剩余候选中随机补选；
...
未设置或留空 `P2D_PORTS` 时，客户端每次启动会从编译候选池中随机选择 3–5 个端口；
也可以显式设置逗号分隔的固定端口。
...
环境文件至少填写 `P2D_DOMAIN`、`P2D_XOR_KEY` 和 `P2D_AUTH_KEY`。DNS 服务器、IP
白名单和自动端口池已有编译默认值，不需要写入环境文件；要覆盖时再取消模板中的对应
注释。该单元使用动态非特权用户，并只授予监听 1024 以下端口所需的
